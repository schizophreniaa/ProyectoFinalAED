...
private void dfsVisit(Vertex<E> vertex, ListLinked<Vertex<E>> visited, ArrayList<E> recorrido) {
    ...
    public boolean esRueda() {
        int n = listVertex.size();
        if (n < 4) return false; // mínima rueda W4
        ...
        while (aux != null) {
            ...
            if (grado == n - 1) {
                ...
                return false; // los otros nodos deberían tener grado 3 (2 ciclo + conexión centro)
            }
            aux = aux.next;
        }

        if (centroCount != 1) return false;

        // Verificamos que los nodos menos el centro formen un ciclo
        Vertex<E> centro = null;
        aux = listVertex.getHead();
        while (aux != null) {
            if (aux.data.listAdj.size() == n - 1) {
                centro = aux.data;
                break;
            }
            aux = aux.next;
        ...
        // Construimos lista de nodos sin el centro
        ListLinked<Vertex<E>> cicloNodos = new ListLinked<>();
        aux = listVertex.getHead();
        while (aux != null) {
            if (!aux.data.equals(centro)) {
                cicloNodos.addLast(aux.data);
            }
            aux = aux.next;
        }

        // Verificamos que estos nodos formen ciclo:
        for (Node<Vertex<E>> node = cicloNodos.getHead(); node != null; node = node.next) {
            int countConexionesSubgrafo = 0;
            Node<Edge<E>> adj = node.data.listAdj.getHead();
            while (adj != null) {
                if (cicloNodos.contains(adj.data.getDestination())) {
                    countConexionesSubgrafo++;
                }
                adj = adj.next;
            }
            if (countConexionesSubgrafo != 2) return false;
        }
